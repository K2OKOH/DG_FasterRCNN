...
from model.da_faster_rcnn.CDAN_loss import CDAN
from model.da_faster_rcnn import CDAN_loss


from model.da_faster_rcnn.DA_CDAN import _ImageDA
from model.da_faster_rcnn.DA_CDAN import _InstanceDA
import time
import pdb
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
        ...
        # tgt_cls_score.size() -> ([300, 9])
        # tgt_pooled_feat.size() -> ([300, 4096])

        # 对S，T标签进行softmax
        softmax_cls_score = nn.Softmax(dim=1)(cls_score)
        softmax_tgt_cls_score = nn.Softmax(dim=1)(tgt_cls_score)

        # 计算熵
        entropy = CDAN_loss.Entropy(torch.cat((softmax_cls_score,softmax_tgt_cls_score),0))
        DA_cst_loss = CDAN_loss.CDAN([pooled_feat, tgt_pooled_feat, softmax_cls_score, softmax_tgt_cls_score], self.RCNN_instanceDA, entropy)
        ...
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
               DA_img_loss_cls,tgt_DA_img_loss_cls,DA_cst_loss # tgt_DA_ins_loss_cls,DA_ins_loss_cls,tgt_DA_cst_loss
